...
void computeAABB(const int pNumPoints, const int pBatchSize, const float* pPoints, const int* pBatchIds, float* pAABBMin, float* pAABBMax);
...
class ComputeAABBOp : public OpKernel {
    public:
        explicit ComputeAABBOp(OpKernelConstruction* context) : OpKernel(context) {
            ...
            OP_REQUIRES(context, batchSize_ > 0, errors::InvalidArgument("ComputeAABBOp expects a positive batch size"));     
        }

        void Compute(OpKernelContext* context) override {
            const Tensor& inPointsTensor=context->input(0);
            OP_REQUIRES(context, inPointsTensor.dims() == 2, errors::InvalidArgument
                ("ComputeAabb expects as input the following dimensions (numPoints, 3)"));
            ...
            OP_REQUIRES(context, inBatchTensor.dims() == 2 &&
                ...
                ("ComputeAabb expects as batch ids input the following dimensions (numPoints, 1)"));
            ...
            computeAABB(numPoints, batchSize_, inPointsPtr, inBatchPtr, outAABBMinPtr, outAABBMaxPtr);
        }
    private:
        int   batchSize_;
};
