...
def compute_aabb(inPts, inBatchIds, batchSize):
    return MCConv_module.compute_aabb(inPts, inBatchIds, batchSize)
tf.NoGradient('ComputeAabb')

def sort_points_step1(inPts, inBatchIds, aabbMin, aabbMax, numCells, batchSize):
    return MCConv_module.sort_points_step1(inPts, inBatchIds, aabbMin, aabbMax, numCells, batchSize)
tf.NoGradient('SortPointsStep1')

def sort_points_step2(inPts, inBatchIds, inFeatures, keys, indexs, numCells, batchSize):
    return MCConv_module.sort_points_step2(inPts, inBatchIds, inFeatures, keys, indexs, numCells, batchSize)
...
def _sort_points_step2_grad(op, *grads):
    ...
    return [inPtsGrad, None, inFeatureGrad, None, None]
...
def find_neighbors(inPts, inBatchIds, inPts2, cellIndexs, aabbMin, aabbMax, radius, batchSize):
    return MCConv_module.find_neighbors(inPts, inBatchIds, inPts2, cellIndexs, aabbMin, aabbMax, radius, batchSize)
tf.NoGradient('FindNeighbors')

def compute_pdf(inPts, inBatchIds, aabbMin, aabbMax, startIndexs, neighbors, window, radius, batchSize):
    return MCConv_module.compute_pdf(inPts, inBatchIds, startIndexs, neighbors, aabbMin, aabbMax, window, batchSize, radius)
tf.NoGradient('ComputePDF')

def poisson_sampling(inPts, inBatchIds, cellIndexs, aabbMin, aabbMax, radius, batchSize):
    return MCConv_module.poisson_sampling(inPts, inBatchIds, cellIndexs, aabbMin, aabbMax, radius, batchSize)
...
def spatial_conv(inPts, inFeatures, inBatchIds, inPDFs, inSamplePts, neighStartIndexs, packedNeighs, aabbMin, aabbMax,
    weights1, weights2, weightsOut, biases1, biases2, biasesOut, numOutFeatures, combin, batchSize, radius):
    return MCConv_module.spatial_conv(inPts, inFeatures, inBatchIds, inPDFs, inSamplePts, neighStartIndexs, packedNeighs, aabbMin, aabbMax,
        weights1, biases1, weights2, biases2, weightsOut, biasesOut, numOutFeatures, combin, batchSize, radius)
@tf.RegisterGradient("SpatialConv")
def _contract_neighs_grad(op, *grads):
